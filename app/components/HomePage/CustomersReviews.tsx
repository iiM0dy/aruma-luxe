    ...
    <section className="py-24 border-t border-white/5">
      <div className="flex flex-col gap-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {reviews.map(review => (
            <div key={review.id} className="group relative flex flex-col gap-8 p-10 rounded-[2rem] bg-white/[0.01] border border-white/5 transition-all duration-300 hover:bg-white/[0.03]">
              <div className="flex gap-1">
                {[...Array(5)].map((_, i) => (
                  <span key={i} className="material-symbols-outlined cursor-default select-none star-filled text-primary !text-xl opacity-80">star</span>
                ))}
              </div>
              ...
              <div className="flex items-center gap-4 mt-auto">
                <div className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-primary font-bold">
                  {review.sender[0]}
                </div>
                <div className="flex flex-col">
                  <h4 className="text-white text-xl font-bold font-amiri">{review.sender}</h4>
                  <div className="flex items-center gap-1.5">
                    <span className="material-symbols-outlined cursor-default select-none text-green-500 !text-sm">verified</span>
              ...
              <div className="absolute top-10 left-10 opacity-[0.03] pointer-events-none">
                <span className="material-symbols-outlined cursor-default select-none !text-9xl">format_quote</span>
              </div>
            </div>
          ))}
        </div>
